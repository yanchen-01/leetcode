...
foreach($list as $value){
	...
	if( !is_dir( $serveroot."/".$value ) ) continue;
...
function publish(){
	...
	$conf = conf('global','server.worknode');
	if(empty($conf)) return;
	
	$servers = explode(',',$conf);//所有需要同步的服务器节点
	foreach($servers as $k=>$v) {
		$v = trim($v);
		if(empty($v))unset($servers[$k]);
	}
...
<title>SVN 同步设置</title>
	...
	<?php if(!empty($admin)){ ?>
	<style>
		#nav{width:95%;margin:0 auto 20px auto;overflow:hidden;}
		#box{
			width:88%;
			height:24px;		
			padding:5px;
    ...
    <link rel="stylesheet" type="text/css" href="/css/admin/header.css" media="all" />
	<?php }else{?>
	<style>
		#box{
...
<body>
	<?php if(!empty($admin)){ ?>
	<?php 
	$html = file_get_contents(DOCUROOT."/template/header_admin.html") ;
	$html = str_replace('{%$smarty.session.NickName%}', $_SESSION['NickName'], $html);
	echo $html;
	?>
	<div id="nav">
		<div class="l">当前位置: <?php echo $servername; ?></div>
		<div class="r">清理数据表结构: http://<?php echo $servername; ?>/include/plugins/cleartb.php?mod=数据对象</div>
	</div>
	<?php } ?>
	<div id='box'>
