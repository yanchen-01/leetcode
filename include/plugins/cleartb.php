...
/**
 * 清理缓存调用测试
 * http://beta.haiwaitel.com/include/plugins/cleartb.php?mod=struct
 * http://beta.haiwaitel.com/include/plugins/cleartb.php?mod=bbs_user
 */
include '../_inc.php';
...
if(empty($_GET['node'])){
	...
	$servers = explode(',',conf('global','server.worknode'));
	echo "Rebuid cache of {$mod}...<hr><br>";
	foreach($servers as $host) {
		if(empty($host)) continue;
		echo file_get_contents("http://{$host}/include/plugins/cleartb.php?node={$host}&mod={$mod}");
	}
