...
class System{
	...
	function clearApiCache(){
		...
		$cacheObj = func_initMemcached($_GlobalConfig['host'],$_GlobalConfig['port']);
		
		$list = $this->getAppNameConfig();
		
		foreach($list as $AppName){
			$apidir = DOCUROOT."/{$AppName}/api";
			if(is_dir($apidir)){
				$apifiles = files::fileList($apidir);
				foreach($apifiles as $file){
					$classname = substr($file,0,-4);
					$cacheID = 'api-'.$AppName.'-'.$classname;
					$result = $cacheObj->delete($cacheID);
					echo $cacheID." ";
					var_dump($result);
				}
			}
