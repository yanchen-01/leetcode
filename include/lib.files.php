...
class files {
	...
	//获得指定路径下的class map
	//files::getClassMap( DOCUROOT."/space/payment/vendor/paypal",array('demo.php','cacert.pem','return.php'),true);
	public static $classmap;
	public static function getClassMap( $path, $skip=array(),$out=false ){
		$list = files::fileAll($path);
		...
		foreach($list as $f){
			...
			if(is_dir($path."/".$f)) {
				files::getClassMap($path."/".$f,$skip);
			}else{
				$key = substr($f,0,strlen($f)-4);
				files::$classmap[$key]=str_replace(DOCUROOT, '', $path)."/".$f;
			}
		}
		
		if($out) foreach(files::$classmap as $item=>$val) echo "'{$item}'=>'$val',<br>";
	}
