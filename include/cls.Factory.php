...
final class Factory {
	...
	function __destruct(){
		...
		if(isset($_GET['debug'])&&isset($_SESSION['UserLevel'])){
			if( $_GET['debug']=='showsql' && $_SESSION['UserLevel']==9 )$this->debug=true;
			if( $_GET['debug']=='showget' && $_SESSION['UserLevel']==9 )debug::d($_GET);
			if( $_GET['debug']=='showpost' && $_SESSION['UserLevel']==9 )debug::d($_POST);
		}
	...
	private function doAction($ACT){
		...
		//是否启用smarty的调试
		if($this->smartydebug) $ACT->smartyDebug();
	...
	private function authGoTo($level){
		...
		if($this->admin){
			go( "/passport/?act=admin&errmsg=accessdeny&checkAuth=true&".$tailstr);
		}
		go( "/passport/?".$tailstr);
	}
