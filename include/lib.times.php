...
class times{
	 ...
	 * @param number $y
	 * @param number $m
	 * @param number $d 默认为0时返回的数据不指定当前日
	 * @return array $calendar
	 */
	static public function calendar($y, $m, $d=0) {
		$currentCheck = true;
		if(empty($d)){
			$currentCheck = false;
			$d = 1;
		...
		$lastMonth = $m > 1 ? ($m - 1) : 12;
		$lastYear = $m > 1?$y:$y-1;
		$nextMonth = $m == 12 ? 1 : ($m + 1);
		$nextYear = $m == 12 ? $y+1: $y;
	
		$firstDay = mktime ( 0, 0, 0, $m, 1, $y );
		$startTime = mktime ( 0, 0, 0, $lastMonth, 15, $lastYear );
		$endTime = mktime ( 0, 0, 0, $nextMonth, 15, $nextYear );
	
		$calendar = [ ];
		$week = [ ];
		for($daytime = $startTime; $daytime < $endTime; $daytime += 24 * 3600) {
			$weekday = date ( 'w', $daytime );
			$day = date ( 'd', $daytime );
			$month = date ( 'm', $daytime );
			$year = date ( 'Y', $daytime );
			$current = ($day == $d && $m == $month && $currentCheck );
			$week [$weekday] = [
					'day' => $day,
					'month' => $month,
					'year' => $year,
			...
			if ($weekday == 6 && $daytime >= $firstDay) {
				ksort($week);
				$calendar [] = $week;
				$week = [];
			}
	
			if ($weekday == 0 && date ( 'm', $daytime ) == $nextMonth)
				break;
		}
	
		$result = [
				'calendar'=>$calendar,
				'last'=>['month'=>$lastMonth,'year'=>$lastYear],
	 ...
	 */
	static public function getDateTimeByZone($timestamp, $timezone="America/Los_Angeles", $format="Y-m-d H:i:s"){
	 ...
	 * 根据指定时区返回某一时刻对应的时间戳
	 */
	static public function getTimeStampByZone($dateTimeStr, $timezone="America/Los_Angeles"){
		if(empty($timezone))$timezone="America/Los_Angeles";
		date_default_timezone_set($timezone);
